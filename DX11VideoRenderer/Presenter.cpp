...
HRESULT DX11VideoRenderer::CPresenter::SetXVPOutputMediaType(IMFMediaType* pType, DXGI_FORMAT vpOutputFormat)
{
    ...
    MFVIDEOFORMAT mfvf = {};
    CAMDDrawMonitorInfo* pMonInfo = NULL; ///// added to have physMonDim early
    HMONITOR hMon = NULL; ///// ditto
    hMon = MonitorFromWindow(m_hwndVideo, MONITOR_DEFAULTTONULL); ///// ditto
    pMonInfo = m_pMonitors->FindMonitor(hMon); ///// ditto
    if (SUCCEEDED(hr))
    {
        hr = MFInitVideoFormat_RGB(
            &mfvf,
            pMonInfo->physMonDim.cx,//m_rcDstApp.right,   // setting initial dimensions to m_rcDstApp limits 
            pMonInfo->physMonDim.cy,//m_rcDstApp.bottom, // video dim zoom-in by the initial video window dims
